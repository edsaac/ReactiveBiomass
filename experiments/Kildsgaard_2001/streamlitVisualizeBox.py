...
with col1:
    ...
    selected_time = st.select_slider(
        " ", all_vtk_paths[1:], format_func=convertPathToTimeLabel)
    st.markdown("----")
...
if selected_chem == "perm":
    with col1:
        log_scale = st.checkbox("Normalized log-scale?", False)
    if log_scale:
        data = np.log10(data/REFERENCE_PERMEABILITY)

if selected_chem == "U":
    with col1:
        u_component = st.selectbox("Component", ["Mag", 0, 1, 2])
    if u_component == "Mag":
