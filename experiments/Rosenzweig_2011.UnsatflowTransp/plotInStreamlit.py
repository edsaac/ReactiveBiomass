...
cols = st.columns(3)
with cols[0]: scalarName = st.selectbox("Select field", options=scalarList, index=0)
with cols[1]: with_logscale = st.checkbox("Log-scale?", False)
with cols[2]: 
    label = st.text_input("Label", value="XAR", max_chars=50)
