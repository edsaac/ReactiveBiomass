...
caseName = st.selectbox(
    "Case:", ["constantHead", "highFlowRate", "oxygenReplenish"], index=1
)
...
def getAllMeshes(field: str):
    ...
    times = [
        float(t)
        for t in subprocess.check_output("foamListTimes", cwd=f"./{caseName}")
        .decode("utf-8")
        .splitlines()
        if t[0:2].isnumeric()
    ]
...
scalarList = sorted(
    "EPS NO3 rH rN XDNp XI BAP NH4 h porosity perm_clog perm_satu XDN kdet XNp tracer POCr XARp XAR rDN Sw XN katt O2 hydraulicCond DOC".split(),
    key=str.casefold,
)
...
fig, (rax, ax) = plt.subplots(
    2, 1, figsize=(8, 7), gridspec_kw={"height_ratios": [1, 5]}, sharex=True
)
fig.set_facecolor("#ffffff00")
ax.stackplot(
    scalar.t / 86400,
...
ax.text(
    12.5,
    2.0e-4,
    r"$X_{\mathsf{Inert},\,\mathsf{labile}}$",
    ha="center",
    va="center",
    fontsize=18,
    rotation=-5,
...
ax.text(
    15.0,
    5.2e-4,
    r"$X_{\mathsf{Inert},\,\mathsf{recalcitrant}}$",
    ha="center",
    va="center",
    fontsize=18,
    rotation=-15,
...
rax.plot(
    scalar.t / 86400,
    np.gradient(np.sum(biomasses, axis=0), scalar.t / 86400),
...
for i, probe in enumerate(probes):
    ax.plot(
        scalar.t / 86400,
        probe / (initConc[i] * 1.0e-3),
        label=rf"$C_{{\mathsf{{{labels[i]}}}}}$",
        color=nutrient_colors[i],
        lw=3,
    )
